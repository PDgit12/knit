 ...
 * `engram_get_workflow({phase})`. This file emits only what *this project*
 * needs to know:
 *
 ...
 *   - phase status
 *
 * Output is wrapped in `<!-- engram:start --> ... <!-- engram:end -->`
...
export const ENGRAM_MARKER_START = '<!-- engram:start -->';
export const ENGRAM_MARKER_END = '<!-- engram:end -->';
...
export function generateClaudeMd(
  ...
  return `${ENGRAM_MARKER_START}\n\n${body}\n\n${ENGRAM_MARKER_END}\n`;
}

/** Replace the engram block inside an existing CLAUDE.md, or append it if no markers exist. */
export function spliceEngramBlock(existing: string, newBlock: string): { content: string; mode: 'replaced' | 'appended' | 'sidecar-needed' } {
  const startIdx = existing.indexOf(ENGRAM_MARKER_START);
  const endIdx = existing.indexOf(ENGRAM_MARKER_END);

  if (startIdx !== -1 && endIdx !== -1 && endIdx > startIdx) {
    // Has both markers — replace block, preserve surrounding content
    const before = existing.slice(0, startIdx);
    const after = existing.slice(endIdx + ENGRAM_MARKER_END.length);
...
${stackDesc}Engram-powered workflow. The protocol depth is fetched on demand via \`engram_get_workflow({phase})\` — this file holds only project-specific facts.`;
}

function generateSessionStartup(): string {
  return `## Session start

First action: call \`engram_load_session\`. One MCP call returns last sessions, handoff, learnings, false positives, project knowledge.
...
function generateDomainArchitecture(config: EngramConfig): string {
  if (!config.domains || config.domains.length === 0) {
    return `## Domain Architecture\n\nNo domains detected. Use \`engram_setup_project\` to describe your project — engram will configure domains and review agents.`;
  }
...
function generateBuildGates(config: EngramConfig): string {
  const gates: string[] = [];
...
Call \`engram_get_workflow({phase: "tier"})\` for the full decision aid.`;
}
...
engram_get_workflow({phase: "plan"})        // PLAN phase + plan-mode rules
...
engram_get_workflow({phase: "ship"})        // commit + ship + production
