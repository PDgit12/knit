/**
 * Workflow protocol content, fetched on demand via engram_get_workflow.
 *
 ...
 * engram_get_workflow(phase) for protocol depth", and the agent pulls
...
export function listWorkflowSections(): { name: string; description: string }[] {
  return [
    ...
    { name: 'tools', description: 'Engram MCP tools reference — what each is for.' },
  ];
}
...
**Navigation:** call \`engram_get_workflow({phase})\` with any of:
- \`overview\` (this) · \`tier\` · \`phases\`
- \`research\` · \`ideate\` · \`plan\` · \`execute\` · \`optimize\` · \`review\`
- \`tdd\` · \`learn\` · \`handoff\` · \`ship\` · \`tools\`

Start of every session: call \`engram_load_session\` first. It returns prior sessions, learnings, false positives, project knowledge in one round trip.
...
- \`engram_find_fanout\` — is the file central?
- \`engram_search_learnings\` — have we done this before?
- \`engram_query_dependents\` — what depends on what I'd touch?
- \`engram_search_sessions\` — did a past session tackle something similar?
...
Each phase has one quality gate. Don't move on until it passes. Call \`engram_get_workflow({phase})\` for the depth on any phase.`;
}
...
**Standard tasks:** read the affected files directly. Check cross-domain ripple via \`engram_query_dependents\`. Pull relevant past sessions via \`engram_search_sessions\` if the area has history.

**Complex tasks:** spawn \`code-explorer\` agents per affected domain in parallel. Each scout reports complexity + risks. Use \`engram_find_fanout\` to spot universal-contract files.
...
**New features:** TDD when applicable (see \`engram_get_workflow({phase: "tdd"})\`). Failing test → minimum impl → refactor.
...
> Project brain available: \`engram_query_dependents\`, \`engram_get_false_positives\`, \`engram_search_learnings\`, \`engram_search_sessions\`. Call them if you need depth.
...
Quality check before \`engram_record_learning\`:
...
- \`engram_record_learning\` — durable, reusable insight (Stripe webhook signature rules).
- \`engram_save_session_summary\` — what this session accomplished (refactored auth middleware). Opt-in, narrative.
...
When context degrades — circular debugging, repeated failures, post-compaction confusion — call \`engram_save_handoff\` with:
...
Then tell the user to start a fresh session. The next session's \`engram_load_session\` will surface the handoff first.
...
function tools(_: WorkflowContext): string {
  return `# Engram MCP tools — reference

**Read the brain** (cheap, instant, no side effects):
| Tool | Use when |
|------|----------|
| \`engram_load_session\` | First call every session. Returns last sessions, handoff, learnings, false positives. |
| \`engram_query_imports\` | Before editing — what depends on this file? |
| \`engram_query_dependents\` | What does this file need to work? |
| \`engram_query_exports\` | Find a function without opening the file. |
| \`engram_query_tests\` | Is this tested? Or: list all untested files. |
| \`engram_find_fanout\` | High-risk files — change carefully. |
| \`engram_search_learnings\` | Did we solve this before? |
| \`engram_search_sessions\` | Did a past session work in this area? |
| \`engram_get_false_positives\` | Filter known non-issues out of review prompts. |
| \`engram_brain_status\` | Brain health + hit-rate + token-accounting. |
| \`engram_get_workflow\` | Fetch protocol depth for a specific phase. |

**Update the brain** (only when there's real signal):
| Tool | Use when |
|------|----------|
| \`engram_classify_task\` | At task start. Returns tier + phases. |
| \`engram_build_context\` | Get cross-domain ripple + pitfalls for the current task. |
| \`engram_record_learning\` | A non-obvious, reusable insight surfaced (pass the quality check first). |
| \`engram_record_false_positive\` | Review agent flagged a non-issue. |
| \`engram_save_session_summary\` | Session accomplished something a future session would search for. |
| \`engram_save_handoff\` | Context degrading — save state for next session. |
| \`engram_setup_project\` | First-time project setup for non-code domains (legal, research, marketing). |

**Team orchestration** (for parallel review boards):
| Tool | Use when |
|------|----------|
| \`engram_get_teams\` | Get auto-detected or custom teams. |
| \`engram_define_team\` | Create a custom team. |
| \`engram_start_team_review\` | Start a parallel review board. |
| \`engram_post_team_findings\` | Each team posts to the shared board. |
| \`engram_get_board_summary\` | Cross-team findings, severity-gated. |`;
}
